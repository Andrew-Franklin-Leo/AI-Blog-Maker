...
const PostCard = ({ id, title, content, created_at, metadata }: PostCardProps) => {
  ...
  return (
    <article className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 focus-within:ring-2 focus-within:ring-blue-500">
      <Link to={`/post/${id}`} className="block h-full outline-none">
        <div className="p-6">
          <header className="mb-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 mb-2 transition-colors">
              {title}
            </h2>
